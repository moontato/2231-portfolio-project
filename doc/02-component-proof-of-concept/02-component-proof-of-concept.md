...
For my design, I think I'm going to have the kernel methods change certain
attributes / variables about the scenario / calculation being run. For example,
my kernel methods would mostly be mutator methods `change___()`, with only one
method being an accessor method `calculatePosition(int t)`. However, I think I
would need to add more--kernel--accessor methods so that the abstract class
could access the data in required fields.

In contrast, the secondary methods will be where most of the calculations are
taking place. These methods will make use of the kernel methods to provide more
information using the same data.
